...
Class A corresponds to the specified execution of the exercise, while the other 4 classes correspond to common mistakes. Sensors were mounted on users’ glove, armband, lumbar belt and dumbbell.
...
OOS error is expected to be around `r round(100 * (1 - mean(modelFit$resample$Accuracy)), 2)`%. For random forests, the out-of-bag(OOB) error estimate gives an unbiased estimate of the test set error.
...
Since we have an overall OOS error rate of `r round(100 * (1 - cfmatrix$overall[1]), 2)`% using random forests which is fairly sufficient, no other methods are investigated.
